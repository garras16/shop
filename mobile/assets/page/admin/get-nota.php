...
$sql = mysql_query("SELECT jual.id_jual,invoice,tgl_nota,tenor,nama_karyawan,pelanggan.id_pelanggan,nama_pelanggan,(qty_ambil*(harga-diskon_rp-diskon_rp_2-diskon_rp_3)) AS total
FROM
    jual
...
if ($sisa_piutang==0) {
	...
	die();
		...
		$sql2=mysql_query("SELECT MAX(tgl_janji_next) AS tgl_janji FROM penagihan_detail WHERE id_jual=$id_jual");
		$row2=mysql_fetch_array($sql2);
