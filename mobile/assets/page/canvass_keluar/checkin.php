...
function get_toko(barcode){
	...
	$('#dummy').load('assets/page/canvass_keluar/get-checkbar.php?kode=' + barcode, function(){
		var toko=$(this).html();
		if (toko==''){
			$('#scan').attr('style', '');
...
function get_pelanggan(barcode){
	$('#dummy').load('assets/page/canvass_keluar/get-info.php?kode=' + barcode, function(){
		var id=$(this).html();
		$('#id_pelanggan').val(id);
		compare_lokasi();
	})
