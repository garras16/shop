...
if (isset($simpan_canvass_siap_kirim_post)){
	$sql = mysql_query("UPDATE canvass_siap_kirim SET status='2' WHERE id_canvass_siap_kirim=$id");
	$sql = mysql_query("SELECT id_jual FROM canvass_siap_kirim WHERE id_canvass_siap_kirim=$id");
	$row=mysql_fetch_array($sql);
	$sql = mysql_query("UPDATE jual SET status_konfirm=7 WHERE id_jual=" .$row['id_jual']. "");
	
	$sql = mysql_query("SELECT id_jual FROM pengiriman WHERE id_jual=" .$row['id_jual']);
	if (mysql_num_rows($sql)>0){
		$sql2 = mysql_query("UPDATE pengiriman SET status=1,jenis='LUAR KOTA',tanggal_kirim='$tanggal',id_karyawan=$id_karyawan WHERE id_jual=" .$row['id_jual']. "");
	} else {
		$sql2 = mysql_query("INSERT INTO pengiriman VALUES(null, " .$row['id_jual']. ", 1, '$tanggal',$id_karyawan,null,null,null,null,'LUAR KOTA')");
	}
...
?>					
			...
			<table class="table table-bordered table-striped" style="margin-top:10px">
				...
				<tbody>
				<?php
				$sql=mysql_query("SELECT nama_barang, nama_satuan, SUM(qty_ambil) AS jumlah, harga, diskon_rp, diskon_rp_2, diskon_rp_3
FROM
    jual_detail
