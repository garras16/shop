...
FROM
 ...
 WHERE id_jual_detail=$id_jual_detail AND barang.status=0");
	...
	$sql2=mysql_query("INSERT INTO canvass_siap_kirim VALUES(null,$id_canvass_keluar,'$tanggal',$id,'0',$id_karyawan)");
	$sql2=mysql_query("SELECT * FROM canvass_siap_kirim WHERE id_jual=$id");
 ...
 AND id_barang_masuk_rak NOT IN (SELECT id_barang_masuk_rak FROM canvass_siap_kirim_detail WHERE id_canvass_siap_kirim=$id_canvass_siap_kirim)");
			while ($row2=mysql_fetch_array($sql2)){
				...
				if ($tmp_qty_ambil>=$stok){
					$qty_ambil_=$stok;
					$sql3=mysql_query("INSERT INTO canvass_siap_kirim_detail VALUES(null,$id_canvass_siap_kirim,$id_jual_detail,$id_barang_masuk_rak,$qty_ambil_,0,'$expire')");
...
if (isset($_GET['del'])){
	$sql=mysql_query("SELECT * FROM canvass_siap_kirim_detail WHERE id_canvass_siap_kirim_detail=" .$_GET['del']. "");
	$row=mysql_fetch_array($sql);
	$id_jual_detail=$row['id_jual_detail'];
	$sql=mysql_query("DELETE FROM canvass_siap_kirim_detail WHERE id_jual_detail=$id_jual_detail");
	_direct("?page=canvass_keluar&mode=konfirm_jual_2&id=$id");
}
...
if (mysql_num_rows($sql)>0){
	...
	$sql2=mysql_query("DELETE FROM canvass_siap_kirim_detail WHERE id_jual_detail=" .$row['id_jual_detail']);
	_alert("Ada barang yang tidak disimpan karena non aktif");
}
	$sql=mysql_query("SELECT * FROM canvass_siap_kirim INNER JOIN canvass_siap_kirim_detail 
		ON (canvass_siap_kirim.id_canvass_siap_kirim = canvass_siap_kirim_detail.id_canvass_siap_kirim) WHERE id_jual=$id");
	while ($row=mysql_fetch_array($sql)){
		$sql2=mysql_query("SELECT * FROM canvass_keluar_barang WHERE id_canvass_keluar=" .$row['id_canvass_keluar']. " AND id_barang_masuk_rak=" .$row['id_barang_masuk_rak']. "");
		$row2=mysql_fetch_array($sql2);
		$stok=$row2['stok']-$row['qty_ambil'];
		$sql2=mysql_query("UPDATE canvass_keluar_barang SET stok=$stok WHERE id_canvass_keluar=" .$row['id_canvass_keluar']. " AND id_barang_masuk_rak=" .$row['id_barang_masuk_rak']. "");
	}
	$sql=mysql_query("UPDATE jual SET status_konfirm=6 WHERE id_jual=$id");
	$sql=mysql_query("UPDATE canvass_siap_kirim SET status='1' WHERE id_jual=$id");
	_direct("?page=canvass_keluar&mode=konfirm_jual");
}
...
WHERE id_jual=$id");
if (mysql_num_rows($sql)=='0'){
	$sql2=mysql_query("DELETE FROM canvass_siap_kirim WHERE id_jual=$id");
}
...
FROM
    ...
    INNER JOIN barang 
		...
		(SELECT id_jual_detail FROM canvass_siap_kirim_detail)");
							(mysql_num_rows($sql)>0 ? $selesai=false : $selesai=true);
