...
$sql=mysql_query("SELECT SUM(qty_cek) AS qty_cek FROM lap_stock_opname WHERE id_canvass_keluar=$canvass AND id_barang=$id");
$r=mysql_fetch_array($sql);
$total_qty_cek=$r['qty_cek'];

$sql=mysql_query("SELECT barang.id_barang,expire,nama_satuan,SUM(stok) AS total
FROM
    canvass_keluar
    INNER JOIN canvass_keluar_barang 
        ON (canvass_keluar.id_canvass_keluar = canvass_keluar_barang.id_canvass_keluar)
...
while($r=mysql_fetch_array($sql)){
$sql2=mysql_query("SELECT SUM(qty_cek) AS qty_cek FROM lap_stock_opname WHERE id_canvass_keluar=$canvass AND id_barang=" .$r['id_barang']. " AND expire='" .$r['expire']. "'");
