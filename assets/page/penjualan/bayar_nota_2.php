...
//-------------------------------------------------------------------------------------------
	...
	$jumlah_bayar_x+=$b3['jumlah_bayar'];
//-------------------------------------------------------------------------------------------
$ci = mysqli_query($con, "SELECT sisa FROM bayar_nota_jual WHERE no_nota_jual='$no_nota_jual' ORDER BY id_bayar DESC LIMIT 1");
...
if (isset($tambah_bayar_nota_jual_post)){
	...
	if (isset($no_retur)){
      ...
      $now = 2;
		...
		if ($jenis =='Transfer'){
			$sql=mysqli_query($con, "INSERT INTO bayar_nota_jual VALUES(null,'$tanggal','$no_nota_jual','Retur',$jumlah_bayar_retur,$status,'$pengirim_nama_bank','$pengirim_nama_rekening','$pengirim_no_rekening','$penerima_nama_bank','$penerima_nama_rekening','$penerima_no_rekening',null,null,$sisa,$now)");
		} else if ($jenis =='Giro'){
			$sql=mysqli_query($con, "INSERT INTO bayar_nota_jual VALUES(null,'$tanggal','$no_nota_jual','Retur',$jumlah_bayar_retur,$status,null,null,null,'$penerima_nama_bank','$penerima_nama_rekening','$penerima_no_rekening','$jatuh_tempo','$keterangan',0,$sis,$now)");
		} else {
			$sql=mysqli_query($con, "INSERT INTO bayar_nota_jual VALUES(null,'$tanggal','$no_nota_jual','Retur',$jumlah_bayar_retur,$status,null,null,null,null,null,null,null,null,null,$sisa,$now)");
		}
...
<script>
    ...
    function saveThis() {
        ...
        $('#retur_content').append(
            ...
            'e="no_retur[]" value="' + no_retur + '" readonly></div><div class="col-xs-6"><' +
            'input class="form-control selretur" style="width:100%" name="jumlah_retur[]" v' +
            'alue="' + jumlah + '" readonly></div>'
        );
        rb += no_retur + ',';
        jumlah_retur += jumlah;
        $('#tot_retur').val(jumlah_retur);
        $('.selretur').inputmask('decimal', {
            allowMinus: false,
            autoGroup: true,
            groupSeparator: '.',
            rightAlign: false,
    ...
    $(document).ready(function () {
        ...
        $('#myModal').on('show.bs.modal', function (e) {
            $('#content').load(
                'assets/page/penjualan/get-retur.php?id_pelanggan=' +
                <?php echo $id_pelanggan ?> + '&id=' + rb
            );
