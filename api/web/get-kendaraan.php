...
<div class="input-group">
	...
	?>
	<input style="width: 30px" id="plat1_2" name="no_plat_1" maxlength="2" placeholder="BE" value="<?php echo $plat[0] ?>" required="required" readonly>&nbsp;
	<input style="width: 50px" id="plat2_2" name="no_plat_2" onKeyPress="if(this.value.length==4) return false;" type="number" placeholder="9999" value="<?php echo $plat[1] ?>" required="required" readonly>&nbsp;
	<input style="width: 40px" id="plat3_2" name="no_plat_3" maxlength="3" placeholder="IDX" value="<?php echo $plat[2] ?>" required="required" readonly>&nbsp;
	<a id="add" class="btn btn-warning btn-xs" onClick="addPlat()">Tambah No Pol</a>
	<a id="save" class="btn btn-warning btn-xs" onClick="savePlat()" style="display: none;">Simpan No Pol</a>
</div>
<div class="input-group" style="display:none" id="datepicker">
	<span class="input-group-addon">Berlaku Mulai</span>
	<input class="form-control" id="berlaku" name="berlaku" value="<?php echo date('Y-m-d') ?>" required="">
	<span class="input-group-addon"><i class="fa fa-star fa-fw" style="color:red"></i></span>
</div>
<div class="input-group">
...
function addPlat(){
	...
	var plat1=$('#plat1_2').val();var plat2=$('#plat2_2').val();var plat3=$('#plat3_2').val();
	$('#dummy').load('api/web/add-plat-detail.php?id=' + id + '&plat1=' + plat1 + '&plat2=' + plat2 + '&plat3=' + plat3);
	$('#plat1_2').val('');
...
function savePlat(){
	if ($('#plat1_2').val().length>0 && $('#plat2_2').val().length>0 && $('#plat3_2').val().length>0){
		...
		$('#dummy').load('api/web/save-plat-detail.php?id=' + id + '&plat1=' + plat1 + '&plat2=' + plat2 + '&plat3=' + plat3 + '&berlaku=' + berlaku);
